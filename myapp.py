...
class LifeAuditApp:
    ...
    def authenticate_user(self, email_input: str, password_input: str):
        ...
        if not is_configured():
            return False, (
                ...
                "Please open 'supabase_client.py' and set your SUPABASE_URL and SUPABASE_KEY."
            ), None
        ...
        try:
            ...
            if "invalid login credentials" in err_msg or "invalid credentials" in err_msg:
                ...
                return False, f"Login error: {str(ex)}", None
    ...
    def register_user(self, email_input: str, password_input: str, confirm_input: str):
        ...
        try:
            ...
            if "already registered" in err_msg or "already exists" in err_msg:
                ...
                return False, f"Signup error: {str(ex)}", None
    ...
    def add_activity_to_db(self, activity, category, duration, activity_date, notes):
        ...
        try:
            ...
            messagebox.showerror("Cloud Sync Error", f"Failed to save activity to cloud: {ex}")
    ...
    def update_activity_in_db(self, activity_id, activity, category, duration, activity_date, notes):
        ...
        try:
            ...
            messagebox.showerror("Cloud Sync Error", f"Failed to update activity: {ex}")
    ...
    def delete_activity_from_db(self, activity_id):
        ...
        try:
            ...
            messagebox.showerror("Cloud Sync Error", f"Failed to delete activity: {ex}")
    ...
    def save_goal(self, category, minutes):
        ...
        try:
            ...
            messagebox.showerror("Cloud Sync Error", f"Failed to save goal: {ex}")
    ...
    def show_splash_screen(self):
        ...
        tk.Label(
            card,
            text="CLOUD TIME & LIFE AUDIT (SUPABASE)",
            bg=COLORS["card"],
            fg=COLORS["muted"],
            font=("Segoe UI", 9, "bold")
        ).pack(pady=(4, 24))
        ...
        self.splash_status_label = tk.Label(
            card,
            text="Connecting to cloud backend...",
            bg=COLORS["card"],
            fg=COLORS["muted"],
    ...
    def animate_splash(self, step):
        ...
        if progress < 25:
            self.splash_status_label.config(text="Initializing environment...")
        elif progress < 50:
            self.splash_status_label.config(text="Connecting to Supabase cloud...")
        elif progress < 80:
            self.splash_status_label.config(text="Preparing auth & secure tables...")
        else:
        ...
        if step < 100:
            ...
            self.root.after(180, self.show_auth_screen)
    ...
    def show_auth_screen(self, initial_mode="login"):
        ...
        self.clear_root()
        self.root.title("Occupied — Cloud Log In or Sign Up")
    ...
    def render_auth_form(self):
        ...
        if self.auth_mode == "login":
            ...
            if not is_configured():
                config_box = tk.Frame(
                    form, bg=COLORS["yellow_card"],
                    highlightbackground=COLORS["yellow_border"],
                    highlightthickness=1, padx=10, pady=6
                )
                config_box.pack(fill="x", pady=(0, 10))
                tk.Label(
                    config_box, text="⚙️ Supabase Setup Required",
                    bg=COLORS["yellow_card"], fg=COLORS["text"], font=("Segoe UI", 8, "bold")
                ).pack(anchor="w")
                tk.Label(
                    config_box, text="Set SUPABASE_URL & SUPABASE_KEY in supabase_client.py",
                    bg=COLORS["yellow_card"], fg=COLORS["muted"], font=("Segoe UI", 8)
                ).pack(anchor="w")
    ...
    def handle_login(self):
        ...
        if hasattr(self, "login_submit_btn") and self.login_submit_btn.winfo_exists():
            self.login_submit_btn.config(text="CONNECTING...", state="disabled")
    ...
    def handle_signup(self):
        ...
        if hasattr(self, "signup_submit_btn") and self.signup_submit_btn.winfo_exists():
            self.signup_submit_btn.config(text="CREATING ACCOUNT...", state="disabled")
            self.root.update_idletasks()
        ...
        messagebox.showinfo(
            "Account Created!",
            f"Account created successfully for {email} on Supabase.\n"
            "Your profile and activities will now persist permanently in the cloud."
        )
        self.start_session(user_or_err, canonical_name)
    ...
    def start_session(self, user_obj, username: str):
        ...
        self.clear_root()
        self.root.title(f"{self.user_name}'s Occupied Dashboard" if self.user_name else "Occupied")
        self.build_ui()
        self.refresh_all()
    ...
    def build_ui(self):
        ...
        tk.Label(
            menu_header,
            text=f"Cloud: {self.current_username}",
            bg="#C5B2DB",
            fg=COLORS["muted"],
    ...
    def show_dashboard(self):
        """Render the Occupied Dashboard with real data metrics and modern cards."""
        print("DASHBOARD VERSION: bold-palette-v1")
        self.clear_content()
        ...
        right = tk.Frame(self.content, bg=COLORS["bg"], width=360)
        right.pack(side="right", fill="both", expand=True, padx=(8, 0))

        # --- LEFT/CENTER COLUMN ---
        # 1. Three bold colored stat cards side by side at top
        self.build_stat_cards_row(left)

        # 2. Activity Chart (60%) + Your Notes (40%) side-by-side row
        chart_notes_row = tk.Frame(left, bg=COLORS["bg"])
        chart_notes_row.pack(fill="both", expand=True, pady=(0, 0))
        chart_notes_row.columnconfigure(0, weight=6)
        chart_notes_row.columnconfigure(1, weight=4)
        chart_notes_row.rowconfigure(0, weight=1)

        chart_frame = tk.Frame(chart_notes_row, bg=COLORS["bg"])
        chart_frame.grid(row=0, column=0, sticky="nsew", padx=(0, 4))

        notes_frame = tk.Frame(chart_notes_row, bg=COLORS["bg"])
        notes_frame.grid(row=0, column=1, sticky="nsew", padx=(4, 0))

        self.build_activity_chart_card(chart_frame)
        self.build_notes_card(notes_frame)
        ...
        self.build_recent_activity_card(right)

        # 3. Top Categories this week
        self.build_top_categories_card(right)
    ...
    def _save_note(self):
        ...
        if not self.current_user_id or not is_configured():
            self._notes_feedback_label.config(
                text="⚠ Not connected to cloud. Configure supabase_client.py first.",
                fg=COLORS["danger"]
            )
            return
    ...
    def show_add_activity(self):
        ...
        self.build_subpage_header(
            "Add Activity",
            "Log what you accomplished, track duration, and save directly to Supabase cloud."
        )
    ...
    def save_activity(self):
        ...
        messagebox.showinfo("Saved!", f"{activity} was synced to your cloud audit.")
        self.show_dashboard()
    ...
    def show_history(self):
        ...
        self.build_subpage_header(
            "Activity History",
            "Select any past activity record to edit details or delete it permanently from Supabase."
        )
    ...
    def delete_selected_activity(self):
        ...
        if activity_id is None:
            return
        if messagebox.askyesno("Delete activity", "Delete this activity permanently from cloud?"):
    ...
    def show_goals(self):
        ...
        self.build_subpage_header(
            "Weekly Goals",
            "Set your target hours per category each week (synced to cloud). Set to 0 to omit from tracking."
        )
    ...
    def save_goals(self):
        try:
            ...
            return
        messagebox.showinfo("Saved", "Your weekly goals have been synced to Supabase.")
        self.show_dashboard()
    ...
    def show_analytics(self):
        ...
        tk.Label(
            right, text="quick audit summary", bg=COLORS["mint_card"],
    ...
    def show_monthly_audit(self):
        ...
        self.build_subpage_header(
            "Monthly Audit",
            "Compare month-over-month performance and discover actionable insights."
        )
    ...
    def show_settings(self):
        ...
        for category in self.custom_categories:
            ...
            self.make_button(
                row, "PICK", lambda v=variable, s=swatch: self.pick_category_color(v, s),
    ...
    def save_settings(self):
        ...
        if invalid:
            messagebox.showwarning("Invalid colour", "Use a valid hex color code like #EA588C.")
            return
        ...
        if self.current_user_id and is_configured():
            try:
                ...
                messagebox.showerror("Cloud Sync Error", f"Failed to update categories on cloud: {ex}")
                return
        ...
        self.root.title(f"{self.user_name}'s Occupied Dashboard" if self.user_name else "Occupied")
        messagebox.showinfo("Saved", "Settings synced to Supabase successfully.")
        self.show_dashboard()
